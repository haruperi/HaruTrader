...
def test_get_market_data_with_mock():
    ...
    with patch('app.core.data_acquisition.MT5Client', return_value=mock_mt5_client):
    ...
    if len(market_data_time) == 0:
        ...
        logger.info(f"Processed {len(market_data_time)} data points for time-based retrieval")
    ...
    if len(market_data_pos) == 0:
        ...
        # Display data information
        logger.info(f"Processed {len(market_data_pos)} data points for position-based retrieval")
